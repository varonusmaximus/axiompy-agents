...
from axiompy.agents.rag.domain.chunker import FixedSizeChunker
...
class TestSentenceChunker:
    ...
    def test_chunk_document_basic(self):
        ...
        from axiompy.agents.rag.domain.chunker import SentenceChunker
...
class TestParagraphChunker:
    ...
    def test_chunk_document_basic(self):
        ...
        from axiompy.agents.rag.domain.chunker import ParagraphChunker
...
class TestChunkerFactory:
    ...
    def test_create_fixed_size(self):
        ...
        from axiompy.agents.rag import ChunkerFactory
        from axiompy.agents.rag.factory import ChunkerSettings, ChunkerType
...
class TestChunkerFactoryIntegration:
    """Tests for factory integration with chunker types."""

    def test_factory_creates_sentence_chunker(self):
        """Test factory creates SentenceChunker."""
        ...
        # Test by ingesting text
        service.ingest_text("First sentence. Second sentence. Third sentence.", source="test")

    def test_factory_creates_paragraph_chunker(self):
        """Test factory creates ParagraphChunker."""
        service = RAGServiceFactory.create(
            embedder_type=EmbedderType.MOCK,
            vector_store_type=VectorStoreType.MOCK,
            llm_provider="mock",
            chunker_type=ChunkerType.PARAGRAPH,
        )

        assert service is not None
        # Test by ingesting text
        service.ingest_text("Para one.\n\nPara two.\n\nPara three.", source="test")


# =============================================================================
# EmbedderFactory Tests
# =============================================================================
...
class TestEmbedderFactory:
    ...
    def test_unknown_type_raises(self):
        ...
        from axiompy.agents.rag.adapters.embedders import EmbedderFactory
        from axiompy.agents.rag.factory import EmbedderSettings
...
class TestVectorStoreFactory:
    ...
    def test_create_memory_via_enum(self):
        ...
        from axiompy.agents.rag.adapters.vector_stores import VectorStoreFactory
        from axiompy.agents.rag.factory import VectorStoreSettings, VectorStoreType
    ...
    def test_unknown_type_raises(self):
        ...
        from axiompy.agents.rag.adapters.vector_stores import VectorStoreFactory
        from axiompy.agents.rag.factory import VectorStoreSettings
...
class TestOpenAIEmbedderFactory:
    ...
    def test_factory_creates_openai_embedder(self):
        """Test factory creates OpenAIEmbedder."""
        from axiompy.agents.rag.adapters.embedders.openai import OpenAIEmbedder
        ...
        service = RAGServiceFactory.create(
            ...
            vector_store_type=VectorStoreType.MOCK,
            llm_provider="mock",
            embedder_settings=EmbedderSettings(api_key="sk-test"),
        )

        assert service is not None
        assert isinstance(service.embedder, OpenAIEmbedder)

    def test_factory_openai_requires_api_key(self):
        """Test that factory requires API key for OpenAI."""
        with pytest.raises(RAGConfigurationError, match="api_key required"):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.OPENAI,
                vector_store_type=VectorStoreType.MOCK,
                llm_provider="mock",
                embedder_settings=EmbedderSettings(),  # No api_key
            )
...
class TestOllamaEmbedderFactory:
    ...
    def test_factory_creates_ollama_embedder(self):
        """Test factory creates OllamaEmbedder."""
        from axiompy.agents.rag.adapters.embedders.ollama import OllamaEmbedder
        ...
        assert service is not None
        assert isinstance(service.embedder, OllamaEmbedder)

    def test_factory_ollama_with_custom_settings(self):
        ...
        assert service is not None
        assert isinstance(service.embedder, OllamaEmbedder)
        assert service.embedder.model == "mxbai-embed-large"
...
class TestFileSystemSource:
    ...
    def test_custom_extensions(self, tmp_path):
        ...
        (tmp_path / "a.custom").write_text("A")
        (tmp_path / "b.txt").write_text("B")

        source = FileSystemSource(extensions={".custom"})
        docs = source.load_documents([str(tmp_path)])
...
class TestRAGService:
    ...
    def mock_service(self):
        """Create RAGService with all mocks."""
        return RAGService(
            document_source=MockDocumentSource(),
            chunker=FixedSizeChunker(_chunk_size=100, _chunk_overlap=20),
            embedder=MockEmbedder(dimension=128),
            vector_store=MockVectorStore(),
            llm_provider=MockLLMProvider(response="Test answer"),
        )

    def test_ingest_text(self, mock_service):
        """Test ingesting raw text."""
        result = mock_service.ingest_text("Some content to ingest", source="test")
        assert result >= 1

    def test_ingest_documents(self, mock_service):
        """Test ingesting Document objects."""
        docs = [
            Document(
                id="d1",
        ...
        stats = mock_service.ingest(docs)

        assert stats["documents_processed"] == 2
        assert stats["chunks_created"] >= 2

    def test_query(self, mock_service):
        """Test querying."""
        mock_service.ingest_text("Python is a programming language.", source="test")

        response = mock_service.query("What is Python?")

        assert isinstance(response, RAGResponse)
        assert response.answer == "Test answer"
        assert response.model == "mock-model"

    def test_search_without_generation(self, mock_service):
        """Test search without LLM generation."""
        mock_service.ingest_text("Python is great for data science.", source="test")

        results = mock_service.search("data science")
    ...
    def test_delete_document(self, mock_service):
        ...
        mock_service.ingest(
            [
                Document(
                    id="to_delete",
                    content="Delete me" * 20,
                    metadata=DocumentMetadata(source="test.txt"),
                )
            ]
        )

        deleted = mock_service.delete_document("to_delete")

        assert deleted >= 1

    def test_invalid_prompt_template_raises(self):
        """Test that invalid prompt template raises error."""
...
class TestRAGServiceFactory:
    ...
    def test_create_mock(self):
        ...
        service = RAGServiceFactory.create_mock()

        assert isinstance(service, RAGService)
        service.ingest_text("Test content", source="test")

    def test_create_mock_with_custom_response(self):
        """Test mock service with custom response."""
        service = RAGServiceFactory.create_mock(mock_response="Custom answer")
        service.ingest_text("Content", source="test")

        response = service.query("Question?")
        assert response.answer == "Custom answer"

    def test_create_with_mock_types(self):
        """Test creating with mock enum types."""
        service = RAGServiceFactory.create(
            embedder_type=EmbedderType.MOCK,
            vector_store_type=VectorStoreType.MOCK,
            llm_provider="mock",
    ...
    def test_create_with_memory_store(self):
        """Test creating with in-memory vector store."""
        service = RAGServiceFactory.create(
            embedder_type=EmbedderType.MOCK,
        ...
        service.ingest_text("Test", source="test")
        assert service.vector_store.chunk_count >= 1

    def test_chunker_settings(self):
        """Test custom chunker settings."""
        service = RAGServiceFactory.create(
            embedder_type=EmbedderType.MOCK,
            vector_store_type=VectorStoreType.MOCK,
            llm_provider="mock",
            chunker_settings=ChunkerSettings(chunk_size=200, chunk_overlap=40),
        )

        assert service.chunker.chunk_size == 200
        assert service.chunker.chunk_overlap == 40

    def test_embedder_settings(self):
        """Test custom embedder settings."""
    ...
    def test_vector_store_settings(self):
        """Test custom vector store settings."""
        settings = VectorStoreSettings(collection_name="test-collection")
        service = RAGServiceFactory.create(
            embedder_type=EmbedderType.MOCK,
            vector_store_type=VectorStoreType.MOCK,
            llm_provider="mock",
            vector_store_settings=settings,
        )
...
class TestRAGIntegration:
    ...
    def test_full_workflow(self):
        ...
        service = RAGServiceFactory.create_mock(mock_response="The answer is 42")

        service.ingest_text(
            "The meaning of life, the universe, and everything is 42.",
            source="hitchhiker.txt",
        )

        response = service.query("What is the meaning of life?")

        assert response.answer == "The answer is 42"
        assert response.has_sources

    def test_multiple_document_ingest(self):
    ...
    def test_search_relevance(self):
        ...
        service.ingest_text("Python is excellent for data science", source="python.txt")
        service.ingest_text("JavaScript is used for web development", source="js.txt")

        results = service.search("data science programming")

        assert len(results) > 0


# =============================================================================
# Factory Edge Cases Tests
# =============================================================================


class TestFactoryEdgeCases:
    """Tests for factory error paths and edge cases."""

    def test_pinecone_missing_api_key(self):
        """Test that Pinecone requires api_key."""
        # Factory wraps errors in RAGConfigurationError
    ...
    def test_pgvector_missing_database_url(self):
        """Test that pgvector requires database_url."""
        with pytest.raises(RAGConfigurationError):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.MOCK,
    ...
    def test_openai_embedder_missing_api_key(self):
        """Test that OpenAI embedder requires api_key."""
        with pytest.raises(RAGConfigurationError):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.OPENAI,
                vector_store_type=VectorStoreType.MOCK,
    ...
    def test_openai_llm_missing_api_key(self):
        """Test that OpenAI LLM requires api_key."""
        with pytest.raises(RAGConfigurationError, match="api_key"):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.MOCK,
                vector_store_type=VectorStoreType.MOCK,
                llm_provider="openai",
    ...
    def test_anthropic_llm_missing_api_key(self):
        """Test that Anthropic LLM requires api_key."""
        with pytest.raises(RAGConfigurationError, match="api_key"):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.MOCK,
                vector_store_type=VectorStoreType.MOCK,
    ...
    def test_unknown_llm_provider(self):
        """Test that unknown LLM provider raises error."""
        with pytest.raises(RAGConfigurationError, match="Unknown LLM provider"):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.MOCK,
                vector_store_type=VectorStoreType.MOCK,
...
# =============================================================================
# Service Edge Cases Tests
# =============================================================================


class TestServiceEdgeCases:
    """Tests for RAGService edge cases and error handling."""
    ...
    def test_ingest_documents_with_paths(self, mock_service):
        """Test ingest_documents method with paths."""
        mock_service.document_source.set_document("path1", "Content 1 " * 20)
        mock_service.document_source.set_document("path2", "Content 2 " * 20)

        count = mock_service.ingest_documents(["path1", "path2"])

        assert count >= 2

    def test_ingest_document_single_path(self, mock_service):
        """Test ingest_document method with single path."""
        mock_service.document_source.set_document("test.txt", "Test content " * 20)

        count = mock_service.ingest_document("test.txt")

        assert count >= 1

    def test_ingest_text_with_metadata(self, mock_service):
        """Test ingest_text with custom metadata."""
        count = mock_service.ingest_text(
            "Content with metadata " * 20,
    ...
    def test_query_with_custom_params(self, mock_service):
        """Test query with custom parameters."""
        mock_service.ingest_text("Test content for query", source="test")

        response = mock_service.query(
            "Test question?",
            top_k=3,
            min_score=0.1,
            temperature=0.5,
    ...
    def test_search_with_min_score(self, mock_service):
        """Test search with min_score filter."""
        mock_service.ingest_text("Test content for search", source="test")

        results = mock_service.search("test", top_k=5, min_score=0.5)

        assert isinstance(results, list)

    def test_invalid_question_placeholder(self):
        """Test that missing {question} in template raises error."""
...
class TestCLI:
    ...
    def test_cli_parser_exists(self):
        ...
        from axiompy.agents.rag.applications.cli import create_parser
    ...
    def test_cli_main_no_command(self):
        ...
        from axiompy.agents.rag.applications.cli import main

        # No command should return error
        result = main([])
        assert result == 1

    def test_cli_main_stats_with_mock(self):
        """Test CLI stats command with mock embedder."""
        from unittest.mock import patch
        from axiompy.agents.rag.applications.cli import main

        # Mock the factory to avoid real service creation
        with patch("axiompy.agents.rag.applications.cli.RAGServiceFactory") as mock_factory:
            mock_service = MockVectorStore()
            mock_factory.create.return_value = RAGServiceFactory.create_mock()

            result = main(["stats", "--embedder", "mock", "--llm", "mock"])
            # Should succeed or fail gracefully
            assert result in [0, 1]

    def test_cli_main_ingest_nonexistent(self):
        """Test CLI ingest with nonexistent file."""
        from axiompy.agents.rag.applications.cli import main
    ...
    def test_cli_ingest_args(self):
        """Test CLI ingest command parsing."""
        from axiompy.agents.rag.applications.cli import create_parser

        parser = create_parser()
        args = parser.parse_args(["ingest", "file1.txt", "file2.md"])

        assert args.command == "ingest"
        assert args.paths == ["file1.txt", "file2.md"]

    def test_cli_query_args(self):
        """Test CLI query command parsing."""
        from axiompy.agents.rag.applications.cli import create_parser
    ...
    def test_cli_query_with_options(self):
        """Test CLI query command with options."""
        from axiompy.agents.rag.applications.cli import create_parser

        parser = create_parser()
        # Options go AFTER the subcommand for argparse
    ...
    def test_cli_chat_args(self):
        """Test CLI chat command parsing."""
        from axiompy.agents.rag.applications.cli import create_parser
    ...
    def test_cli_stats_args(self):
        """Test CLI stats command parsing."""
        from axiompy.agents.rag.applications.cli import create_parser
    ...
    def test_cli_subcommand_options(self):
        """Test CLI subcommand-level options parsing."""
        from axiompy.agents.rag.applications.cli import create_parser

        parser = create_parser()
        # Options are on subparsers, not the main parser
        args = parser.parse_args(
            [
                "stats",
                "--embedder",
        ...
        assert args.embedder == "fastembed"
        assert args.store == "memory"
        assert args.llm == "mock"
        assert args.model_cache_dir == "/tmp/models"

    def test_cli_ingest_with_chunk_options(self):
        """Test CLI ingest with chunking options."""
        from axiompy.agents.rag.applications.cli import create_parser
...
class TestReasoningAdapter:
    ...
    def test_generate_error(self):
        ...
        from unittest.mock import Mock
        from axiompy.agents.rag.adapters.llm import ReasoningAdapter
        from axiompy.agents.rag.errors import RAGLLMError
...
class TestChromaVectorStore:
    ...
    def test_init_ephemeral(self, chroma_available, unique_collection):
        ...
        from axiompy.agents.rag.adapters.vector_stores.chroma import ChromaVectorStore
...
class TestPineconeVectorStore:
    ...
    def test_init(self):
        """Test initialization."""
        from axiompy.agents.rag.adapters.vector_stores.pinecone import PineconeVectorStore
...
class TestPineconeVectorStoreFactory:
    ...
    def test_factory_creates_pinecone(self):
        """Test factory creates PineconeVectorStore."""
        from axiompy.agents.rag.adapters.vector_stores.pinecone import PineconeVectorStore
        ...
        assert service is not None
        assert isinstance(service.vector_store, PineconeVectorStore)

    def test_factory_pinecone_requires_api_key(self):
        """Test that factory requires API key for Pinecone."""
        with pytest.raises(RAGConfigurationError, match="api_key required"):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.MOCK,
                vector_store_type=VectorStoreType.PINECONE,
    ...
    def test_factory_pinecone_requires_host(self):
        """Test that factory requires host for Pinecone."""
        with pytest.raises(RAGConfigurationError, match="host"):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.MOCK,
                vector_store_type=VectorStoreType.PINECONE,
                llm_provider="mock",
...
class TestPGVectorStore:
    ...
    def test_init_missing_database_url(self):
        ...
        from axiompy.agents.rag.adapters.vector_stores.pgvector import PGVectorStore
    ...
    def test_init_psycopg2_not_installed(self, monkeypatch):
        ...
        try:
            ...
            # Clear cached import
            import importlib
            from axiompy.agents.rag.adapters.vector_stores import pgvector
...
class TestPGVectorStoreFactory:
    ...
    def test_factory_pgvector_requires_database_url(self):
        """Test that factory requires database_url for pgvector."""
        with pytest.raises(RAGConfigurationError, match="database_url required"):
            RAGServiceFactory.create(
                embedder_type=EmbedderType.MOCK,
                vector_store_type=VectorStoreType.PGVECTOR,
                llm_provider="mock",
...
class TestChromaVectorStoreFactory:
    ...
    def test_factory_creates_chroma(self, chroma_available, unique_collection):
        """Test factory creates ChromaVectorStore."""
        service = RAGServiceFactory.create(
            embedder_type=EmbedderType.MOCK,
            vector_store_type=VectorStoreType.CHROMA,
            llm_provider="mock",
            vector_store_settings=VectorStoreSettings(collection_name=unique_collection),
        )
    ...
    def test_factory_creates_chroma_with_settings(
        self, chroma_available, tmp_path, unique_collection
    ):
        """Test factory with VectorStoreSettings."""
        settings = VectorStoreSettings(
        ...
        assert service.vector_store.collection_name == unique_collection


# =============================================================================
# Additional InMemoryVectorStore Tests
# =============================================================================
...
class TestLLMSettings:
    ...
    def test_default_values(self):
        """Test default values."""
        from axiompy.agents.rag.factory import LLMSettings
...
class TestAPISettings:
    ...
    def test_default_values(self):
        """Test default values."""
        from axiompy.agents.rag.applications.api import APISettings

        settings = APISettings()
        assert settings.host == "0.0.0.0"
        assert settings.port == 8080
        assert settings.vector_store_type == VectorStoreType.MEMORY
        assert settings.persist_path is None

    def test_chroma_auto_persist_path(self):
        """Test Chroma auto-sets persist_path in __post_init__."""
        from axiompy.agents.rag.applications.api import APISettings, DEFAULT_PERSIST_PATH

        settings = APISettings(vector_store_type=VectorStoreType.CHROMA)
        assert settings.persist_path == DEFAULT_PERSIST_PATH

    def test_chroma_custom_persist_path(self):
        """Test Chroma with custom persist_path."""
        from axiompy.agents.rag.applications.api import APISettings
        ...
        assert settings.persist_path == "./custom_path"

    def test_from_env_defaults(self, monkeypatch):
        """Test from_env with defaults (no env vars set)."""
        from axiompy.agents.rag.applications.api import APISettings

        # Clear any existing env vars
        for key in ["RAG_HOST", "RAG_PORT", "RAG_EMBEDDER", "RAG_STORE"]:
    ...
    def test_from_env_custom(self, monkeypatch):
        ...
        monkeypatch.setenv("RAG_STORE", "chroma")
        monkeypatch.setenv("RAG_PERSIST_PATH", "/data/rag")

        settings = APISettings.from_env()
        assert settings.host == "127.0.0.1"
        assert settings.port == 9000
        assert settings.vector_store_type == VectorStoreType.CHROMA
        assert settings.persist_path == "/data/rag"
...
class TestRAGAPIService:
    """Tests for RAGAPIService."""

    def test_create_mock(self):
        """Test create_mock returns functional service."""
        from axiompy.agents.rag.applications.api import RAGAPIService

        service = RAGAPIService.create_mock()
        assert service is not None
        assert service._settings.embedder_type == EmbedderType.MOCK
        assert service._settings.vector_store_type == VectorStoreType.MOCK
    ...
    def test_get_app(self):
        """Test get_app returns FastAPI app."""
        from axiompy.agents.rag.applications.api import RAGAPIService
    ...
    def test_ingest_text_endpoint(self):
        ...
        response = client.post(
            "/ingest/text",
            json={"text": "This is test content.", "source": "test.txt"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "ok"
    ...
    def test_search_endpoint(self):
        ...
        client.post(
            "/ingest/text",
            json={"text": "Python is a programming language.", "source": "test.txt"},
        )
    ...
    def test_missing_question_returns_error(self):
        """Test query/search without question returns error."""
        from axiompy.agents.rag.applications.api import RAGAPIService
...
class TestCLIPersistence:
    """Tests for CLI persistence options."""

    def test_cli_chroma_store_option(self):
        """Test CLI accepts chroma store option."""
        from axiompy.agents.rag.applications.cli import create_parser
        ...
        args = parser.parse_args(
            ["ingest", "./docs/", "--store", "chroma", "--persist-path", "./data"]
        )
        assert args.store == "chroma"
        assert args.persist_path == "./data"

    def test_cli_collection_option(self):
        """Test CLI accepts collection option."""
        from axiompy.agents.rag.applications.cli import create_parser

        parser = create_parser()
        args = parser.parse_args(["query", "What is X?", "--collection", "my_collection"])
        assert args.collection == "my_collection"

    def test_create_rag_service_with_chroma(self):
        """Test create_rag_service with chroma options."""
        pytest.importorskip("chromadb")
        from axiompy.agents.rag.applications.cli import create_parser, create_rag_service

        parser = create_parser()
        # Use mock embedder to avoid loading real model
        args = parser.parse_args(
            [
                "ingest",
                "./docs/",
        ...
        rag = create_rag_service(args)
        assert rag is not None


# =============================================================================
# Source Adapters Tests
# =============================================================================
...
class TestSourceFactory:
    ...
    def test_source_type_enum_values(self):
        ...
        from axiompy.agents.rag.adapters.sources import SourceType
...
class TestURLSource:
    ...
    def test_url_source_initialization(self):
        ...
        from axiompy.agents.rag.adapters.sources.url import URLSource
    ...
    def test_strip_html_tags(self):
        ...
        from axiompy.agents.rag.adapters.sources.url import _strip_html_tags
    ...
    def test_extract_title(self):
        ...
        from axiompy.agents.rag.adapters.sources.url import _extract_title
...
class TestObjectStoreSource:
    ...
    def test_object_store_source_initialization_mocked(self):
        ...
        with patch(
            "axiompy.agents.rag.adapters.sources.object_store.ObjectStorageFactory"
        ) as mock_factory:
            mock_storage = Mock()
            mock_factory.create.return_value = mock_storage

            from axiompy.agents.rag.adapters.sources.object_store import ObjectStoreSource
...
class TestDatabaseSource:
    ...
    def test_database_source_initialization_mocked(self):
        ...
        with patch("axiompy.agents.rag.adapters.sources.database.DatabaseFactory") as mock_factory:
            mock_db = Mock()
            mock_factory.create.return_value = mock_db

            from axiompy.agents.rag.adapters.sources.database import DatabaseSource
...
class TestPDFSource:
    ...
    def test_pdf_source_requires_pypdf(self):
        ...
        try:
            ...
            from axiompy.agents.rag.adapters.sources.pdf import PDFSource
            from axiompy.agents.rag.errors import RAGIngestionError
    ...
    def test_pdf_source_load_mocked(self):
        ...
        try:
            with patch("axiompy.agents.rag.adapters.sources.pdf.PdfReader") as mock_reader_class:
